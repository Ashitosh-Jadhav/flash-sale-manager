// ============================================
// k6 Load Test — Flash Sale ASYNC Order Endpoint
// ============================================
// This script tests POST /api/orders/async which is the new
// Redis-backed asynchronous order flow.
//
// Usage: k6 run flash-sale-async.js --vus 10 --duration 10s
// ============================================
...
export function setup() {
  ...
  const productPayload = JSON.stringify({
    name: `Async Load Test Product ${Date.now()}`,
    description: 'Product for async load testing',
    price: 99.99,
    stock: 100000,
    flash_sale: true,
  });
  ...
  console.log(`Created async test product ID: ${productData.data.id} with 100,000 stock`);
...
export default function (data) {
  ...
  const params = {
    headers: {
      ...
      'Idempotency-Key': `k6-async-${__VU}-${__ITER}-${Date.now()}`,
    },
  };

  const res = http.post(`${BASE_URL}/api/orders/async`, payload, params);

  // Track custom metrics
  orderLatency.add(res.timings.duration);

  // Verify response
  // Async endpoint returns 202 Accepted, or 400 for errors like invalid data
